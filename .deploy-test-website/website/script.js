...
if (proofOverview && matchMedia("(min-width:801px)").matches) {
  ScrollTrigger.create({
    ...
    snap: {
      snapTo: (value) => Math.round(value * 6) / 6,
...
const appTrigger = window.__anikaSafari
  ? null
  : ScrollTrigger.create({
      trigger: appSection,
      start: "top top",
...
if (productParade && !window.__anikaSafari) {
  ...
  rows.forEach((row, index) => {
    ...
    const distance = reversed ? 90 : -90;
    const setHint = (target, value) =>
      gsap.set(target, { willChange: value });
    const clearHint = (target) =>
      gsap.set(target, { clearProps: "willChange" });
    /* Product renders must never depend on scroll progress for completeness.
       The old scrubbed clip-path could leave an image looking half-loaded when
       a wheel/trackpad jump stopped between trigger boundaries. */
    gsap.set(media, { x: 0, opacity: 1, clipPath: "none" });
    const copyObserver = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting) return;
        copyObserver.disconnect();
        setParadeChapter(index);
        setHint(copy, "transform, opacity");
        gsap.fromTo(
          copy,
          { x: -distance * 0.55, opacity: 0 },
          {
            x: 0,
            opacity: 1,
            duration: 0.78,
